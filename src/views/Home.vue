<template>
  <div class="home">
    ...
    <div class="" v-for="(item,index) in depositList" :key="index">
      <div class="poolBox dBox" v-if="activeTab==0 || (activeTab==1&&item.status==2) ||(activeTab==2&&item.status==1) ||(activeTab==3&&item.status==0) || (activeTab==4 && item.user.toLowerCase() == account.toLowerCase())" >
        ...
        <div class="btnWrap" v-if="item.status == 1">
          ...
          <el-button class="pbtn mr10" type="default" v-if="item.user.toLowerCase() == account.toLowerCase()"  @click="closeProject(item.id)">关闭众筹</el-button>
      </div>
      </div>
    </div>
...
import { formatAmount, accMul, accDiv, toFixed, accAdd, accSub } from '@/utils/format.js'
...
export default {
  ...
  data() {
    return {
      depositList: [],
      balance: 0,
      crowdfundingContract:'0x68833DAFe292466c20D075a92dD501FCf2E986fB',
  ...
  methods: {
    ...
    async closeProject(index){
      ...
      const code = myContract.methods.closeProject(index).encodeABI();
      this.web3.eth
        .sendTransaction({
          from: this.account,
          to: this.crowdfundingContract,
          data: code,
        })
    ...
    async contribution(item,index){
      ...
      const amount = accMul(item.inputAmount,1e18)
      const code = myContract.methods.contribution(index).encodeABI();
      this.web3.eth
        .sendTransaction({
          from: this.account,
          to: this.crowdfundingContract,
    ...
    confirm(){
      ...
      maxAmount = accMul(maxAmount,1e18)
      const code = myContract.methods.add(to, maxAmount, maxPeople).encodeABI();
      this.web3.eth
        .sendTransaction({
          from: this.account,
          to: this.crowdfundingContract,
